    ...
    if (downloadBtn) {
        downloadBtn.addEventListener('click', function() {
            ...
            try {
                ...
                // Construir ruta al original: webp/*.webp -> originals/*.ext
                const swiperEl = document.querySelector('.eventSwiper');
                const originalExt = swiperEl && swiperEl.dataset.originalExt ? swiperEl.dataset.originalExt : 'jpg';
                const originalPrefix = swiperEl && swiperEl.dataset.originalPrefix ? swiperEl.dataset.originalPrefix : '';
                const originalDir = swiperEl && swiperEl.dataset.originalDir ? swiperEl.dataset.originalDir : 'originals';

                let originalSrc;
                if (originalPrefix) {
                    // Extraer número del nombre del webp y construir nombre original
                    const match = imgSrc.match(/(\d+)\.webp$/);
                    const num = match ? parseInt(match[1], 10) : 1;
                    const dir = imgSrc.substring(0, imgSrc.lastIndexOf('/'));
                    originalSrc = dir.replace('/webp', '/' + originalDir) + '/' + originalPrefix + num + '.' + originalExt;
                } else {
                    originalSrc = imgSrc.replace('/webp/', '/' + originalDir + '/').replace(/\.webp$/, '.' + originalExt);
                }
                console.log('Original source:', originalSrc); // Debug

                // Extraer nombre del archivo para el download
